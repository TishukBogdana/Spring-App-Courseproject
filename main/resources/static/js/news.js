...
new Vue({
    ...
    created: function () {
        ...
        var art =  document.querySelector("div.news");
        fetch('http://localhost:8080/news/findfresh',{method:'GET'}).then(function (resp) {
            return resp.json();
        }).then(function (data) {
            ...
            fetch("http://localhost:8080/nwscom/getall",{
                    method:'GET'
                }
            ).then(function (resp) {
                return resp.json();
    ...
    methods:{
        getByName: function () {
            ...
            var com =  document.querySelector("div.comment");
            while(com.firstChild) com.removeChild(com.firstChild);
            var name  = document.getElementById('begin').value;
            alert(name);
            fetch('http://localhost:8080/news/getbyname?name='+name,{method:'GET'}).then(function (resp) {
                return resp.json();
            }).then(function (data) {
                console.log(data);
                for (var i=0;i<data.length;i++){
